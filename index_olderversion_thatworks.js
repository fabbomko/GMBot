...
const prefix = "!gm ";
const ALLOWED_ROLE_ID = "1146413264650125342"; // Allowed role ID
...
client.on("message", (message) => {
  ...
  if (message.guild && message.guild.id === "1304251077041979472") { //if the user is in Felinia's comp
    if (!member.roles.cache.has("1304550152450736191")) {
    return message.reply("You don't have permission to use this command."); //they don't have the admin role
    }
  }
  
  if (message.guild && message.guild.id === "1131317607539167234") { //if the user is in GM
    if (!member.roles.cache.has(ALLOWED_ROLE_ID)) {
    return message.reply("You don't have permission to use this command."); //they don't have Prod role
    }
  }
  ...
  switch (command) {
    case "judge":
      ...
      submissions[submissionIndex] = new Submission();
      previousMessage.submissionIndex = submissionIndex;
      console.log("Identifier:", previousMessage.submissionIndex);
      submissionIndex++;
      break;

    case "average":
      if (submissions.length === 0) { //will access property of Competition object tmrw ig
