...
const fs = require('fs');
const gameSettings = require("../gm-shop/gameSettings.js");
const { memberHasProducerRole } = require("../gm-shop/playerLookup.js");
const registerVoting = require("./voting.js");
let settings = gameSettings.loadSettings(); //GM34
...
module.exports = {
  ...
  setupClient(client) {
    ...
    registerVoting(client, (guildId) => competitions.get(guildId));
  },
};
async function messageHandler(message) {
  ...
  if (message.content.toLowerCase().startsWith("!gm lockvote on")) { //Locking votes
      if (memberHasProducerRole(message, Comp?.adminRoleID)) {
          gameSettings.setLockvote(message.guild.id, true);
...
if (message.content.toLowerCase().startsWith("!gm submit")) { //gm submit area
  ...
  //list is for general purposes. Not on startup.
    ...
    if (message.member.roles.cache.has(Comp.adminRoleID)) {
      ...
      voting.archiveAndClearRound(message.guild.id);
      settings = gameSettings.loadSettings();

      Comp.submissions = new Array();
      const nextChallenge = gameSettings.getChallengeNumber(message.guild.id);
      message.channel.send(
        `**Reset complete.** Challenge ${nextChallenge - 1} archived to history. Now on Challenge **${nextChallenge}**. Votes, advantages, and live scores cleared.`
      );
...
async function awaitUserReply(channel, userId, timeoutMs) {
...
async function unlockVoting(message, unionName) {
  const guildId = message.guild.id;
  const parsedUnion = gameSettings.parseUnionUfeInput(unionName);
  gameSettings.setUnionUfe(guildId, parsedUnion);
  gameSettings.setLockvote(guildId, false);
  settings = gameSettings.loadSettings();

  const displayUnion = gameSettings.formatUnionUfe(parsedUnion);
  const logchannel = message.client.channels.cache.get("1391418940223393903");
  logchannel?.send(
    `LockVOTE is OFF! 🔓 \n**Current Union UFE:** ${displayUnion}`
  );

  await message.reply(
    "MAKE SURE TO DO `!gm reset` TO CLEAR THE VOTES FROM THE PREVIOUS ELIMINATION IF YOU HAVEN'T ALREADY BEFORE ANY PLAYERS VOTE!!"
  );
  return message.reply(
    `**✅ Voting is UNlocked.** Union set to: **${displayUnion}**`
  );
}
